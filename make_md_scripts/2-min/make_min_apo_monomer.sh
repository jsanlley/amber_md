...
&cntrl
  ...
  restraintmask=':1-306 & !@H=', ! strong restraint on all non-hydrogen atoms (p.344)
  maxcyc=500, ! minimum number of minimizaion cycles (p.344)
  ntmin=1, ! flag for the method of minimization (p.344)
  ncyc=500, ! switching from steepest descent to conjugate gradient (p.344)
/
...
#soft minimization on backbone heavy atoms
 &cntrl
  ...
  restraintmask=':1-306@CA,N,C,O', ! moderate restraint on all non-hydrogen atoms (p.344)
  maxcyc=500, ! minimum number of minimizaion cycles (p.344)
  ntmin=1, ! flag for the method of minimization (p.344)
  ncyc=500, ! switching from steepest descent to conjugate gradient (p.344)
/
...
pmemd.cuda -O -i min5.mdin -o $1_min5.mdout -p $1_solvated.prmtop -c $1_min3.rst -r $1_min5.rst -ref $1_min3.rst -inf $1_min5.info
EOF
